"""
Test cases for SCINet imputation model.
"""
...
class TestSCINet(unittest.TestCase):
    logger.info("Running tests for an imputation model SCINet...")

    # set the log and model saving path
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_IMPUTATION, "SCINet")
    model_save_name = "saved_scinet_model.pypots"

    # initialize an Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # initialize a SCINet model
    scinet = SCINet(
        DATA["n_steps"],
        DATA["n_features"],
        n_stacks=2,
    ...
    def test_1_impute(self):
        imputation_results = self.scinet.predict(TEST_SET)
        assert not np.isnan(
        ...
        logger.info(f"SCINet test_MSE: {test_MSE}")
    ...
    def test_4_lazy_loading(self):
        ...
        imputation_results = self.scinet.predict(GENERAL_H5_TEST_SET_PATH)
        assert not np.isnan(
            imputation_results["imputation"]
        ).any(), "Output still has missing values after running impute()."
        ...
        logger.info(f"Lazy-loading SCINet test_MSE: {test_MSE}")
