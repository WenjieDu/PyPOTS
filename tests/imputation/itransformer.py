"""
Test cases for iTransformer imputation model.
"""
...
class TestiTransformer(unittest.TestCase):
    logger.info("Running tests for an imputation model iTransformer...")

    # set the log and model saving path
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_IMPUTATION, "iTransformer")
    ...
    def test_4_lazy_loading(self):
        ...
        imputation_results = self.itransformer.predict(GENERAL_H5_TEST_SET_PATH)
        assert not np.isnan(
        ...
        logger.info(f"Lazy-loading iTransformer test_MSE: {test_MSE}")
