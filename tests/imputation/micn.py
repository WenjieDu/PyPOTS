"""
Test cases for MICN imputation model.
"""
...
class TestMICN(unittest.TestCase):
    logger.info("Running tests for an imputation model MICN...")

    # set the log and model saving path
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_IMPUTATION, "MICN")
    ...
    def test_0_fit(self):
        self.micn.fit(TRAIN_SET, VAL_SET)

    @pytest.mark.xdist_group(name="imputation-micn")
    def test_1_impute(self):
        imputation_results = self.micn.predict(TEST_SET)
        assert not np.isnan(
    ...
    def test_4_lazy_loading(self):
        self.micn.fit(GENERAL_H5_TRAIN_SET_PATH, GENERAL_H5_VAL_SET_PATH)
        imputation_results = self.micn.predict(GENERAL_H5_TEST_SET_PATH)
        assert not np.isnan(
            imputation_results["imputation"]
        ).any(), "Output still has missing values after running impute()."
