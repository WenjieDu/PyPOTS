"""
Test cases for Crossformer anomaly detection model.
"""
...
from pypots.anomaly_detection import Crossformer
...
class TestCrossformer(unittest.TestCase):
    logger.info("Running tests for an anomaly detection model Crossformer...")

    # Define where to save logs and models
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_ANOMALY_DETECTION, "Crossformer")
    model_save_name = "saved_crossformer_model.pypots"

    # Instantiate a custom Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # Initialize the Crossformer anomaly detection model
    ...
    @pytest.mark.xdist_group(name="anomaly-detection-crossformer")
    def test_0_fit(self):
        """Test training the model on in-memory data."""
        self.crossformer.fit(TRAIN_SET, VAL_SET)

    @pytest.mark.xdist_group(name="anomaly-detection-crossformer")
    def test_1_detect(self):
        """Test anomaly detection and evaluate accuracy, precision, recall, and F1."""
        anomaly_detection_results = self.crossformer.predict(TEST_SET)
        ...
        logger.info(f"Crossformer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
    ...
    def test_3_saving_path(self):
        ...
        self.crossformer.save(saved_model_path)
        self.crossformer.load(saved_model_path)
    ...
    def test_4_lazy_loading(self):
        ...
        anomaly_detection_results = self.crossformer.predict(GENERAL_H5_TEST_SET_PATH)
        anomaly_labels = TEST_SET["anomaly_y"].flatten()
        ...
        logger.info(
            f"Lazy-loading Crossformer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}"
        )
