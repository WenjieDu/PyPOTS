"""
Test cases for ImputeFormer anomaly detection model.
"""
...
from pypots.anomaly_detection import ImputeFormer
...
class TestImputeFormer(unittest.TestCase):
    logger.info("Running tests for an anomaly detection model ImputeFormer...")

    # set the log and model saving path
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_ANOMALY_DETECTION, "ImputeFormer")
    model_save_name = "saved_imputeformer_model.pypots"

    # initialize an Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # initialize an ImputeFormer model
    ...
    @pytest.mark.xdist_group(name="anomaly-detection-imputeformer")
    def test_0_fit(self):
        self.imputeformer.fit(TRAIN_SET, VAL_SET)

    @pytest.mark.xdist_group(name="anomaly-detection-imputeformer")
    def test_1_detect(self):
        anomaly_detection_results = self.imputeformer.predict(TEST_SET)
        ...
        logger.info(f"ImputeFormer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
    ...
    def test_4_lazy_loading(self):
        ...
        anomaly_detection_results = self.imputeformer.predict(GENERAL_H5_TEST_SET_PATH)
        anomaly_labels = TEST_SET["anomaly_y"].flatten()
        ...
        logger.info(
            f"Lazy-loading ImputeFormer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}"
        )
