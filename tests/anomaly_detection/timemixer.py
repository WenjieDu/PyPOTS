"""
Test cases for TimeMixer anomaly detection model.
"""
...
from pypots.anomaly_detection import TimeMixer
...
class TestTimeMixer(unittest.TestCase):
    logger.info("Running tests for an anomaly detection model TimeMixer...")

    # set the log and model saving path
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_ANOMALY_DETECTION, "TimeMixer")
    model_save_name = "saved_timemixer_model.pypots"

    # initialize an Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # initialize an TimeMixer model
    timemixer = TimeMixer(
        DATA["n_steps"],
        DATA["n_features"],
    ...
    @pytest.mark.xdist_group(name="anomaly-detection-timemixer")
    def test_0_fit(self):
        self.timemixer.fit(TRAIN_SET, VAL_SET)

    @pytest.mark.xdist_group(name="anomaly-detection-timemixer")
    def test_1_detect(self):
        anomaly_detection_results = self.timemixer.predict(TEST_SET)
        ...
        logger.info(f"TimeMixer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
    ...
    def test_4_lazy_loading(self):
        ...
        anomaly_detection_results = self.timemixer.predict(GENERAL_H5_TEST_SET_PATH)
        anomaly_labels = TEST_SET["anomaly_y"].flatten()
        ...
        logger.info(
            f"Lazy-loading TimeMixer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}"
        )
