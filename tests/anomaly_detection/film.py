"""
Test cases for FiLM anomaly detection model.
"""
...
from pypots.anomaly_detection import FiLM
...
class TestFiLM(unittest.TestCase):
    logger.info("Running tests for an anomaly detection model FiLM...")

    # Define where to save logs and models
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_ANOMALY_DETECTION, "FiLM")
    model_save_name = "saved_film_model.pypots"

    # Instantiate a custom Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # Initialize the FiLM anomaly detection model
    film = FiLM(
    ...
    @pytest.mark.xdist_group(name="anomaly-detection-film")
    def test_0_fit(self):
    ...
    def test_1_detect(self):
        ...
        anomaly_detection_results = self.film.predict(TEST_SET)
        ...
        logger.info(f"FiLM Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
    ...
    def test_4_lazy_loading(self):
        ...
        anomaly_detection_results = self.film.predict(GENERAL_H5_TEST_SET_PATH)
        anomaly_labels = TEST_SET["anomaly_y"].flatten()
        ...
        logger.info(f"Lazy-loading FiLM Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
