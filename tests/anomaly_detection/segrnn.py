"""
Test cases for SegRNN anomaly detection model.
"""
...
from pypots.anomaly_detection import SegRNN
...
class TestSegRNN(unittest.TestCase):
    logger.info("Running tests for an anomaly detection model SegRNN...")

    # set the log and model saving path
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_ANOMALY_DETECTION, "SegRNN")
    model_save_name = "saved_segrnn_model.pypots"

    # initialize an Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # initialize an SegRNN model
    segrnn = SegRNN(
        DATA["n_steps"],
        DATA["n_features"],
    ...
    @pytest.mark.xdist_group(name="anomaly-detection-segrnn")
    def test_0_fit(self):
        self.segrnn.fit(TRAIN_SET, VAL_SET)

    @pytest.mark.xdist_group(name="anomaly-detection-segrnn")
    def test_1_detect(self):
        anomaly_detection_results = self.segrnn.predict(TEST_SET)
        ...
        logger.info(f"SegRNN Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
    ...
    def test_4_lazy_loading(self):
        ...
        anomaly_detection_results = self.segrnn.predict(GENERAL_H5_TEST_SET_PATH)
        anomaly_labels = TEST_SET["anomaly_y"].flatten()
        ...
        logger.info(f"Lazy-loading SegRNN Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
