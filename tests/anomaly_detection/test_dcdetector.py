...
from pypots.anomaly_detection.dcdetector import DCdetector


class TestDCdetector(unittest.TestCase):
    """Test suite for the DCdetector anomaly detection model."""
    ...
    def test_04_anomaly_rate(self):
        """Test that detected anomaly rate is close to configured rate."""
        result = self.model.predict(self.test_set)
        detected_rate = result["anomaly_labels"].mean()
        self.assertAlmostEqual(detected_rate, self.anomaly_rate, delta=0.05)

    def test_05_invalid_train_set_type(self):
        """Test that non-dict train_set raises TypeError."""
        with self.assertRaises(TypeError):
            self.model.fit([1, 2, 3])

    def test_06_missing_x_key(self):
        """Test that missing 'X' key raises KeyError."""
