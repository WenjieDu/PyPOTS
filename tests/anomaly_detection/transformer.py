"""
Test cases for Transformer anomaly detection model.
"""
...
from pypots.anomaly_detection import Transformer
...
class TestTransformer(unittest.TestCase):
    logger.info("Running tests for an anomaly detection model Transformer...")

    # Define where to save logs and models
    saving_path = os.path.join(RESULT_SAVING_DIR_FOR_ANOMALY_DETECTION, "Transformer")
    model_save_name = "saved_transformer_model.pypots"

    # Instantiate a custom Adam optimizer
    optimizer = Adam(lr=0.001, weight_decay=1e-5)

    # Initialize the Transformer anomaly detection model
    ...
    @pytest.mark.xdist_group(name="anomaly-detection-transformer")
    def test_0_fit(self):
    ...
    def test_1_detect(self):
        ...
        anomaly_detection_results = self.transformer.predict(TEST_SET)
        ...
        logger.info(f"Transformer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}")
    ...
    def test_4_lazy_loading(self):
        ...
        anomaly_detection_results = self.transformer.predict(GENERAL_H5_TEST_SET_PATH)
        anomaly_labels = TEST_SET["anomaly_y"].flatten()
        ...
        logger.info(
            f"Lazy-loading Transformer Accuracy: {accuracy}, F1: {f1}, Precision: {precision}, Recall: {recall}"
        )
