...
class BackboneSegRNN(nn.Module):
    def __init__(self, n_steps: int, n_features: int, seg_len: int = 24, d_model: int = 512, dropout: float = 0.5):
        ...
        if n_steps % seg_len:
            raise ValueError(
                "The argument seg_len is necessary for SegRNN need to be divisible by the sequence length n_steps."
            )
    ...
    def forward(self, x):
        ...
        pos_emb = (
            torch.cat(
                [
                    ...
                    self.channel_emb.unsqueeze(1).repeat(1, self.seg_num, 1),
                ],
                dim=-1,
            )
        ...
        _, hy = self.rnn(pos_emb, hn.repeat(1, 1, self.seg_num).view(1, -1, self.d_model))  # bcm,1,d  1,bcm,d
