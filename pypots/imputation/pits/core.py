"""
The core wrapper assembles the submodules of PITS imputation model
...
from ...nn.modules.patchtst import PatchEmbedding, PredictionHead
from ...nn.modules.pits import PITSBackbone
from ...nn.modules.saits import SaitsLoss, SaitsEmbedding


class _PITS(ModelCore):
    def __init__(
        self,
        n_steps: int,
        n_features: int,
        ...
        # PITS: replace the transformer encoder (with cross-patch attention) with a simple
        # per-patch MLP backbone that processes each patch independently
        self.backbone = PITSBackbone(n_layers, d_model, d_ffn, dropout)
        self.head = PredictionHead(d_model, n_patches, n_steps, dropout)
        self.output_projection = nn.Linear(d_model, n_features)
        # apply SAITS loss function to PITS on the imputation task
    ...
    def forward(
        ...
        # WDU: the original PITS paper isn't proposed for imputation task. Hence the model doesn't take
        ...
        # PITS independent patch processing (no cross-patch attention)
        enc_out = self.backbone(enc_out)  # [bz, d_model, d_model, n_patches]
